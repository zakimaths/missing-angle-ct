...
for(const key of ['ta','tb','tc','chest-64','chest-96','abdomen-400','abdomen-480','synthetic']){
 ...
 sources[key]={input_sha256:crypto.createHash('sha256').update(JSON.stringify(input)).digest('hex'),reference_sha256:crypto.createHash('sha256').update(JSON.stringify(reference)).digest('hex')};
...
if(check){
 ...
 if(expected.schema!==report.schema||expected.algorithm!==report.algorithm||JSON.stringify(expected.sources)!==JSON.stringify(sources)||expected.results.length!==results.length)throw Error('Benchmark provenance or configuration differs.');
