...
api.useInput=async(data,key)=>{
 input=data;reference=null;regions=[];api.reset();$('run-name').value=data.name.slice(0,120);referenceMessage='No reference loaded. Ray errors remain available; image-error comparison requires an aligned reference.';render();const ticket=++generation;
