...
function render(){
 $('reference-info').textContent=referenceMessage;
 if(!result)return;
...
api.reset=()=>{result=null;history=[];$('evaluation').hidden=true;$('label-tools').hidden=true;};
...
$('add-label').onclick=()=>{try{const r={name:$('region-name').value.trim(),x:Number($('roi-x').value)/100,y:Number($('roi-y').value)/100,w:Number($('roi-w').value)/100,h:Number($('roi-h').value)/100};regions=validateLabels({name:$('run-name').value,regions:[...regions,r]}).regions;$('label-message').textContent='Region added. Labels and region errors are included in your saved run.';annotations();}catch(e){$('label-message').textContent=e.message;}};
